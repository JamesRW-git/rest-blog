...
1. The Client sends a POST request to ```/api/users``` to make a new UserIndex
2. On successful creation, server redirects to ```/oauth/token``` with UserIndex credentials
...
5. The Resource Owner (new UserIndex) is now 'logged in' and may begin making requests to protected endpoints.
...
#### ***Of Note:*** 
If the server shuts down, the tokens become invalid. The UserIndex must "log in" again.
