...
2. Follow the same pattern to integrate `UserIndex` repository/controller.
...
2. Use a Fetch POST request on `/api/users` to create a few `UserIndex` records and verify they are in the database.


3. Then, use another POST request to create Posts.
    - Keep in mind that you MUST have an existing `UserIndex` associated with the post
    - Also, make sure to give each new `Post` a category
    
**Things to test**:

- What happens if I create a `Post` with no `UserIndex`?
  

- What happens if I create a `Post` with an invalid `UserIndex`?
  

- Create a `Post` with an invalid `Category`. What happens?


- Can I delete a `UserIndex` which has `Post` records associated?
